# ATM Simulation with Currency Conversion and Daily Withdrawal Limit
...
# Step 4: Define daily withdrawal limit and tracking variable
daily_withdrawal_limit = 50000.00  # Limit in Tshs
daily_withdrawn_amount = 0.00      # Tracks the total withdrawn amount for the day

# Step 5: Create a loop for the ATM menu
while True:
        ...
        if choice == '1':
            ...
            if withdrawal_amount > 0:
                ...
                if withdrawal_in_tshs <= balance_amount:
                    if daily_withdrawn_amount + withdrawal_in_tshs <= daily_withdrawal_limit:
                        balance_amount -= withdrawal_in_tshs
                        daily_withdrawn_amount += withdrawal_in_tshs
                        print(f"Successfully withdrew {withdrawal_amount:.2f} {current_currency}.")
                        converted_balance = balance_amount * exchange_rates[current_currency]
                        print(f"New balance is: {converted_balance:.2f} {current_currency}")
                        print(f"Total withdrawn today: {daily_withdrawn_amount:.2f} Tshs")
                    else:
                        print(f"Withdrawal denied. Daily limit of {daily_withdrawal_limit:.2f} Tshs exceeded.")
                else:
